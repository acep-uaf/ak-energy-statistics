...
l3_check_outliers <- function(path_in, path_config, output_log_path, path_out) {
  ...
  outliers <- l2_indexed %>%
    select(.row_id, project_code, sales_reporting_name, calendar_year, calendar_month, all_of(columns_to_check)) %>%
    pivot_longer(
    ...
    select(.row_id, project_code, sales_reporting_name, calendar_year, calendar_month, column_tested, raw_value, median_val, mad_score, anomaly_severity) %>%
